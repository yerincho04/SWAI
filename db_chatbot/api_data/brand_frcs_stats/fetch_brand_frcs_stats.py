#!/usr/bin/env python3
"""Fetch FTC API: brand-wise franchise store stats, save raw JSON, and export selected fields."""
...
def resolve_service_key(explicit_key: str | None) -> str:
    ...
    key = (
        os.getenv("FTC_BRAND_FRCS_STATS_KEY")
        or os.getenv("FTC_SERVICE_KEY")
        or os.getenv("serviceKey")
    )
    if not key:
        raise RuntimeError(
            "Missing API key. Set FTC_BRAND_FRCS_STATS_KEY (preferred), "
            "or FTC_SERVICE_KEY, or pass --service-key."
        )
    return key.strip()
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Fetch Brand FRCs Stats API and export selected columns.")
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("db_chatbot/api_data/brand_frcs_stats/output"))
    ...
    raw_path = args.output_dir / "brand_frcs_stats_raw.json"
    selected_path = args.output_dir / "brand_frcs_stats_selected.json"
    xlsx_path = args.output_dir / "brand_frcs_stats.xlsx"
